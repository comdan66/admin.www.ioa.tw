...
class Homes extends Admin_controller {
  ...
  public function __construct () {
    ...
    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'status', 'show')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->obj = Article::find ('one', array ('conditions' => array ('id = ? AND status != ? AND type = ?', $id, Article::STATUS_1, Article::TYPE_1))))))
  ...
  public function index ($offset = 0) {
    ...
    $objs = conditions ($searches, $configs, $offset, 'Article', array ('order' => 'id DESC'), function ($conditions) {
      OaModel::addConditions ($conditions, 'status != ? AND type = ?', Article::STATUS_1, Article::TYPE_1);
      return $conditions;
    });
