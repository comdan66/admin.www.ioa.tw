...
class Stars extends Admin_controller {
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'admin/stars';
    $this->icon = 'icon-star-full';
    $this->title = '重要事件';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'status', 'show')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->obj = Article::find ('one', array ('conditions' => array ('id = ? AND status != ? AND type = ?', $id, Article::STATUS_1, Article::TYPE_7))))))
  ...
  public function index ($offset = 0) {
    ...
    $objs = conditions ($searches, $configs, $offset, 'Article', array ('order' => 'id DESC'), function ($conditions) {
      OaModel::addConditions ($conditions, 'status != ? AND type = ?', Article::STATUS_1, Article::TYPE_7);
      return $conditions;
    });
  ...
  public function create () {
    ...
    $validation = function (&$posts) {
      if (!(isset ($posts['status']) && is_string ($posts['status']) && is_numeric ($posts['status'] = trim ($posts['status'])) && in_array ($posts['status'], array_keys (Article::$statusNames)))) $posts['status'] = Article::STATUS_3;
